...
### Fixed — DOM mapper: props-only components unselectable in iframe (2026-05-31)
- **A props-only component (no slot children) rendered among decorative siblings couldn't be selected by clicking the preview** (only via the tree). `DomMapper` bound such a wrapping component to its first DOM sibling, but `SectionMain` emits empty `aria-hidden` `section-pattern` divs flanking the slot content — so the component's nodeId landed on a decorator that isn't an ancestor of the visible content, and `getClosestMappedElement` walking up from a click never reached it. The wrapper search now skips `aria-hidden`/empty decorative candidates and binds to the first content-bearing sibling (falling back to the next sibling when none qualifies), consuming through it so skipped decorators aren't re-matched. Verified: a top-level `<CardFeatured eyebrow=… title=… description=… />` is now selectable in the iframe. (`packages/injected/src/dom-mapper.ts`.)
  - Known limitation: the same props-only component nested inside another slot-bearing component (e.g. `<Grid><CardFeatured …/></Grid>`) still resolves only up to the outer section when clicked — a deeper positional/structural-index match (the rendered DOM carries `data-astro-source-loc`) is the proper fix and is deferred. Tree selection works in all cases.

### Fixed — DOM mapper (iframe selection / content editing) (2026-05-31)
- **Pages with classless wrapper elements were largely unselectable.** `DomMapper` anchored mostly on shared CSS classes, so a chain like `Layout > main` (no class) `> SectionMain > div` (no class) `> Grid > CardFeatured` mapped only a handful of the live DOM nodes — iframe clicks resolved to nothing and the properties panel never populated. Two anchoring fixes: (1) static nodes with element children now prefer a same-tag DOM candidate whose subtree actually has content over an empty decorative sibling of the same tag (e.g. a page `<div>` rendered as a grid section's slot, flanked by empty `section-pattern` divs); (2) transparent-component detection accepts a tag-only match when the component's first child is a classless landmark (e.g. `<main>`) whose tag is unique among the candidate siblings. Verified: `test-page-creation` went from ~3 mapped nodes to the full tree; class-rich pages unchanged. (`packages/injected/src/dom-mapper.ts`.)
- **Sibling elements with identical tag + classes could mis-map.** `scoreMatch` now adds a text-content tiebreaker: when an AST node is a text leaf, an exact (whitespace-normalized) copy match scores +3, so two `<p class="text-muted">` with different copy are told apart by their text rather than left to positional order. Additive — awarded only on exact equality, so it can only break ties, never reject a structural match. (`packages/injected/src/dom-mapper.ts` + `ASTNodeLike.textContent`.)
